...
Singleton {
    ...
    Component.onDestruction: {
        ...
        const processes = [nmStateMonitor, nmDeviceMonitor, wifiScanner, wifiConnector, wifiDisconnector, networkForgetter, macAddressSetter];
    ...
    Process {
        ...
        stdout: SplitParser {
            ...
            onRead: line => {
                ...
                if (line.includes("StateChanged") || line.includes("PrimaryConnectionChanged") || line.includes("WirelessEnabled") || (line.includes("ActiveConnection") && line.includes("State"))) {
    ...
    Process {
        ...
        command: lowPriorityCmd.concat(["nmcli", "-t", "-f", "DEVICE,TYPE,STATE", "device"])
        running: false
        ...
        stdout: SplitParser {
            ...
            onRead: line => {
                if (line.startsWith("yes:")) {
                    const rest = line.slice("yes:".length);   // "70:MyWifi"
                    const parts = root.splitNmcliFields(rest); // ["70","MyWifi"]
    ...
    function setProxyConfig(connectionName, method, httpProxy, httpsProxy, ftpProxy, socksProxy, noProxy) {
        ...
        if (method === "manual") {
            if (httpProxy)
                cmd.push("proxy.http", httpProxy);
            if (httpsProxy)
                cmd.push("proxy.https", httpsProxy);
            if (ftpProxy)
                cmd.push("proxy.ftp", ftpProxy);
            if (socksProxy)
                cmd.push("proxy.socks", socksProxy);
            if (noProxy)
                cmd.push("proxy.no-proxy", noProxy);
        } else if (method === "auto") {}
