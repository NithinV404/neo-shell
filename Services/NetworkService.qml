...
Singleton {
    ...
    readonly property var supportedSecurityTypes: [
        {
            key: "open",
            name: "wifi.panel.security-open"
        },
        {
            key: "wep",
            name: "wifi.panel.security-wep"
        },
        {
            key: "wpa-psk",
            name: "wifi.panel.security-wpa"
        },
        {
            key: "wpa2-psk",
            name: "wifi.panel.security-wpa23"
        },
        {
            key: "sae",
            name: "wifi.panel.security-wpa3"
        },
        {
            key: "wpa-eap",
            name: "wifi.panel.security-wpa-ent"
        },
        {
            key: "wpa2-eap",
            name: "wifi.panel.security-wpa2-ent"
        },
        {
            key: "wpa3-eap",
            name: "wifi.panel.security-wpa3-ent"
        }
    ]
    ...
    function getSignalInfo(signal, isConnected) {
        ...
        if (!icon) {
            icon = signal >= 80 ? "signal_wifi_4_bar" : signal >= 60 ? "network_wifi_3_bar" : signal >= 35 ? "network_wifi_2_bar" : signal >= 15 ? "network_wifi_1_bar" : "signal_wifi_0_bar";
        }
        return {
            icon,
            label
    ...
    function getStatusText(showSpeed = false) {
        ...
        if (root.connecting) {
            return root.connectingTo ? "common.connecting" + " " + root.connectingTo : "common.connecting";
        }

        if (NetworkService.airplaneModeEnabled) {
            return "toast.airplane-mode.title";
        }
        if (!root.wifiEnabled) {
            return "";
        }
            ...
            onStreamFinished: {
                if (text.trim()) {
                    ...
                    if (text.indexOf("Secrets were required") !== -1 || text.indexOf("no secrets provided") !== -1) {
                        root.lastError = "toast.wifi.incorrect-password";
                        forget(connectProcess.ssid);
                    } else if (text.indexOf("No network with SSID") !== -1) {
                        root.lastError = "toast.wifi.network-not-found";
                    } else if (text.indexOf("Timeout") !== -1) {
                        root.lastError = "toast.wifi.connection-timeout";
                    } else {
                        root.lastError = "toast.wifi.connection-failed";
                    }
