...
Singleton {
    ...
    readonly property real volume: {
        ...
        if (vol === undefined || isNaN(vol))
            return 0;
        const maxVolume = Settings.audioVolumeOverdrive ? 1.5 : 1.0;
    ...
    readonly property real stepVolume: Settings.audioVolumeStep / 100.0
    ...
    IpcHandler {
        ...
        function _increment(step: string): string {
            ...
            const newVolume = Math.max(0, Math.min(Settings.audioVolumeOverdrive ? 150 : 100, currentVolume + stepValue));
