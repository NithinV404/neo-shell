...
Singleton {
    ...
    function getMappedBacklightDevice(outputName): string {
        ...
        var mappings = Settings.brightness.backlightDeviceMappings || [];
    ...
    function setMappedBacklightDevice(outputName, devicePath): void {
        ...
        var mappings = SettingsbacklightDeviceMappings || [];
        var nextMappings = [];
        var replaced = false;
        ...
        Settings.brightness.backlightDeviceMappings = nextMappings;
    }
    ...
    function getBrightnessIcon(brightness): string {
        var val = Math.max(0.0, Math.min(1.0, brightness));
    ...
    component Monitor: QtObject {
        ...
        readonly property bool isDdc: Settings.brightness.enableDdcSupport && root.ddcMonitors.some(m => m.connector === modelData.name)
        ...
        readonly property real stepSize: 2 / 100.0
        ...
        function initBrightness(): void {
            ...
            if (isAppleDisplay) {
                ...
                var probeScript = ["preferred=\"$1\"", "if [ -n \"$preferred\" ] && [ ! -d \"$preferred\" ]; then preferred=\"/sys/class/backlight/$preferred\"; fi", "selected=\"\"", "if [ -n \"$preferred\" ] && [ -f \"$preferred/brightness\" ] && [ -f \"$preferred/max_brightness\" ]; then selected=\"$preferred\"; else for dev in /sys/class/backlight/*; do if [ -f \"$dev/brightness\" ] && [ -f \"$dev/max_brightness\" ]; then selected=\"$dev\"; break; fi; done; fi", "if [ -n \"$selected\" ]; then echo \"$selected\"; cat \"$selected/brightness\"; cat \"$selected/max_brightness\"; fi"].join("; ");
